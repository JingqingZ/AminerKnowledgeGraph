...
def algorithm1(keyword_file, simplified_publication_file, output1, output2, output3, output4):
    ...
    curkeywordslist = []
    skip = False
    infile2 = open(simplified_publication_file, 'r')
    for line in infile2:
        if curline % 10000 == 0:
            print(curline)
        if skip:
...
def main():
    algorithm1('../results/publication.keywords_merge', '../results/publication_simplified.data_merge', 
                '../results/author_year_keywords.list', '../results/link_author.list',
                '../results/link.list', '../results/link_diff.list')
